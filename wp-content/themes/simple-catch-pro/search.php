...
get_header(); 
	...
	if (have_posts()): ?>
    	...
    	<header class="page-header">
			<h1 class="page-title"><?php printf( __( 'Showing results for: <span>%s</span>', 'simplecatch' ), get_search_query() ); ?></h1>
		</header>
        ...
        <section class="search-not-found">	
            <article class="post">
                <header class="entry-header">
                    <h1 class="entry-title"><?php printf( __( 'Your search <span> "%s" </span> did not match any documents', 'simplecatch' ), get_search_query() ); ?></h1>
                </header>
                <div class="entry-content clearfix"> 
                    <p><?php _e( 'A few suggestions', 'simplecatch' ); ?></p>
                    <ul>
